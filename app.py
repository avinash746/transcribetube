...
language = st.selectbox("Select Video Language:", ["en", "es", "fr", "de", "hi","it", "ja", "ko", "pt", "ru", "zh-CN"])
...
# # Prompt template
# prompt = """You are a YouTube video summarizer. Summarize the video transcript in points within 250 words."""
...
#     try:
#         response = genai.generate_text(model="gemini-pro", prompt=prompt + transcript_text)
#         return response['text']
#     except Exception as e:
...
# def get_binary_file_downloader_html(text, file_label='Download Text', file_name='summary.txt'):
#     bin_file_base64 = base64.b64encode(text.encode()).decode()
#     href = f'<a href="data:application/octet-stream;base64,{bin_file_base64}" download="{file_name}">{file_label}</a>'
#     return href
...
#                 st.markdown("## Summary:")
#                 st.write(summary)
#                 st.markdown(get_binary_file_downloader_html(summary), unsafe_allow_html=True)
#     except IndexError:
#         st.warning("Invalid YouTube URL. Please enter a valid URL.")
